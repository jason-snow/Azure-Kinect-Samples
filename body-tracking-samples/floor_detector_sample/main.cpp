...
bool s_visualizeJointFrame = true;
    ...
    for (uint32_t i = 0; i < numBodies; i++)
    {
        ...
        for (int joint = 0; joint < static_cast<int>(K4ABT_JOINT_COUNT); joint++)
        {
            if (body.skeleton.joints[joint].confidence_level >= K4ABT_JOINT_CONFIDENCE_LOW)
            {
                ...
                printf("body joint detected !! (%f, %f, %z)\n", jointPosition.xyz.x, jointPosition.xyz.y, jointPosition.xyz.z);
            }
        }
...
void PlayFromDevice(InputSettings inputSettings) {
    ...
    while (s_isRunning)
    {
        ...
        window3d.SetFloorRendering(true, -0.0513381213, 1.15985036, 1.65356004, 0.0438376032, -0.990396202, -0.131124914);
